...
const HomePage = (props) => {
  ...
  const allowedFileTypes =
    "application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf,text/plain,application/vnd.oasis.opendocument.text,application/rtf,image/*";
  ...
  useEffect(() => {
    ...
    if (props.searchTerm.length > 3) {
      API.get(`/search/?term=` + props.searchTerm).then((result) =>
  ...
  const getFiles = (folder, files) => {
    console.log("getFiles called with folder:", folder);
    ...
    const fileResults = files.filter(function (file) {
      console.log(
        `Checking file ${file.name}: parentID="${file.parentID}" vs folder.id="${folder.id}" match=${file.parentID == folder.id}`,
      );
      if (file.parentID == folder.id) {
        return file;
      }
    });
    console.log("FileResults after filter:", fileResults);
    console.log("FileResults length:", fileResults.length);
    setCurrentFileSystem(fileResults);
  };

  function openInNewTab(url) {
    //TODO perhaps context menu to open in new tab?
  ...
  const handleFolderCreate = () => {
    ...
    API.post("/folder/?path=" + filePath + "&folder=" + folderName).then(
      (postResult) => {
        //TODO give response to folder create
        API.get(`/documents/filesystem`).then((result) => {
          checkData(result.data);
        }); //TODO reset path to the new folder? Will be hard to do
      },
    );
  };
