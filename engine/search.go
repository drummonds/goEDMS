...
func SearchExactPhrase(phrase string, index bleve.Index) (*bleve.SearchResult, error) {
	...
	Logger.Debug("Search Results for phrase search", "results", searchResultsQuery)
	if err != nil {
		Logger.Error("SearchPhrase failed with error", "error", err)
		return nil, err
	}
...
func SearchExactSingleTerm(term string, index bleve.Index) (*bleve.SearchResult, error) {
	...
	Logger.Debug("Search Results for single term", "results", searchResults)
	if err != nil {
		Logger.Error("SearchTerm failed with error", "error", err)
		return nil, err
	}
...
func SearchGeneralPhrase(phrase string, index bleve.Index) (*bleve.SearchResult, error) {
	...
	searchQuery := bleve.NewSearchRequest(phraseQuery)
	searchResultsQuery, err := index.Search(searchQuery)
	Logger.Debug("Search Results for term", "results", searchResultsQuery)
	if err != nil {
		Logger.Error("SearchPhrase failed with error", "error", err)
		return nil, err
	}
...
func ParseSearchResults(results *bleve.SearchResult, db database.DBInterface) ([]database.Document, error) {
	var documentIDs []string
	searchHits := results.Hits
	for _, hit := range searchHits {
		Logger.Debug("Search found a hit", "id", hit.ID)
		documentIDs = append(documentIDs, hit.ID)
	}
	documentResults, _, err := database.FetchDocuments(documentIDs, db)
	if err != nil {
		Logger.Error("Unable to fetch documents from database", "error", err)
		return nil, err
	}
